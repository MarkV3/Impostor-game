...
const HostSetup: React.FC = () => {
	...
	const [minPlayersStr, setMinPlayersStr] = useState('4');
	const [answeringSecondsStr, setAnsweringSecondsStr] = useState('60');
	const [votingSecondsStr, setVotingSecondsStr] = useState('90');
	...
	const handleCreate = (e: React.FormEvent) => {
		...
		const parsedAnswering = Math.max(10, parseInt(answeringSecondsStr || '0', 10) || 0);
		const parsedVoting = Math.max(10, parseInt(votingSecondsStr || '0', 10) || 0);
		...
		setAnsweringSecondsStr(String(parsedAnswering));
		setVotingSecondsStr(String(parsedVoting));
		socket.emit('room:create', { minPlayersToStart: parsedMinPlayers, answeringSeconds: parsedAnswering, votingSeconds: parsedVoting, totalRounds: parsedTotalRounds }, (res?: any) => {
			setBusy(false);
			...
			<div className="w-full max-w-2xl">
				...
				<Card>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div>
							<label className="block text-sm font-medium text-gray-300 mb-1">Answering Time (sec)</label>
							<input
								type="text"
								inputMode="numeric"
								pattern="[0-9]*"
								value={answeringSecondsStr}
								onChange={e => setAnsweringSecondsStr(e.target.value.replace(/[^0-9]/g, ''))}
								onBlur={() => {
									const n = parseInt(answeringSecondsStr || '0', 10) || 0;
									const clamped = Math.max(10, n || 10);
									setAnsweringSecondsStr(String(clamped));
								}}
								className="w-full bg-gray-700 text-white p-3 rounded-lg border-2 border-gray-600 focus:border-indigo-500 focus:ring-indigo-500"
							/>
						</div>
						<div>
							<label className="block text-sm font-medium text-gray-300 mb-1">Voting Time (sec)</label>
							<input
								type="text"
								inputMode="numeric"
								pattern="[0-9]*"
								value={votingSecondsStr}
								onChange={e => setVotingSecondsStr(e.target.value.replace(/[^0-9]/g, ''))}
								onBlur={() => {
									const n = parseInt(votingSecondsStr || '0', 10) || 0;
									const clamped = Math.max(10, n || 10);
									setVotingSecondsStr(String(clamped));
								}}
								className="w-full bg-gray-700 text-white p-3 rounded-lg border-2 border-gray-600 focus:border-indigo-500 focus:ring-indigo-500"
							/>
						</div>
